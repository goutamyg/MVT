...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'nfs', 'lasot', 'uav']:
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'nfs', 'lasot', 'uav']:
        if seq.dataset == 'uav':
            base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name[4:])
        else:
...
def run_sequence(seq: Sequence, tracker: Tracker, backend, debug=False, num_gpu=8):
    ...
    output = tracker.run_sequence(seq, backend, debug=debug)
    """
    if debug:
...
def run_dataset(dataset, trackers, backend, debug=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, backend, debug=debug)
    elif mode == 'parallel':
