...
def build_neck(cfg, hidden_dim):
    ...
    if cfg.MODEL.NECK.TYPE in ["BN_FEATURE_FUSOR_LIGHTTRACK"]:
...
def build_feature_fusor(cfg, num_features):

    if cfg.MODEL.NECK.TYPE == "BN_FEATURE_FUSOR_LIGHTTRACK":

        # feature_fusor = build_subnet_feat_fusor(cfg, )
        feature_fusor = PW_Corr_adj(num_kernel=num_features,
                                    adj_channel=cfg.MODEL.HEAD.NUM_CHANNELS)
        return feature_fusor
    else:
        raise ValueError("NECK TYPE %s is not supported." % cfg.MODEL.NECK.TYPE)
