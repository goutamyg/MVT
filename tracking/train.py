...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='mobilevit_track', help='training script name')
    parser.add_argument('--config', type=str, default='mobilevit_256_128x1_got10k_ep100_cosine_annealing', help='yaml configure file')
...
def main():
    ...
    if args.mode == "single":
        ...
        """
        run_training(script_name = args.script,
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d " \
                    "--nnodes %d --node_rank %d lib/train/run_training.py --script %s --config %s --save_dir %s " \
                    "--use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d --distill %d --script_teacher %s " \
                    "--config_teacher %s" \
